...
export default defineContentScript({
  ...
  main() {
    ...
    function showOverlay(payload: NotificationPayload): void {
      ...
      const notification = container.querySelector(".petmood-notification, .petmood-running");
      if (notification) {
        (notification as HTMLElement).style.pointerEvents = "auto";
    ...
    function createHTML(payload: NotificationPayload): string {
      ...
      // All other types: bubble + pet with animation class
      return `
    ...
    function getOverlayStyles(): string {
      return `
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .anim-bounce .petmood-pet {
          animation: bounceJump 0.6s ease-in-out 0.5s infinite alternate;
        }
        @keyframes bounceIn {
        ...
        .anim-float .petmood-pet {
          animation: floatDrift 2.5s ease-in-out infinite alternate;
        }
        .anim-float .petmood-bubble {
          animation: floatDrift 2.5s ease-in-out 0.3s infinite alternate;
        }
        @keyframes floatIn {
        ...
        /* ===== 5. Wobble (슬픔 — 흔들흔들) ===== */
        .anim-wobble {
          animation: wobbleIn 0.5s ease-out forwards;
        }
        ...
        /* ===== 6. Spin (화남 — 빙글 등장) ===== */
        .anim-spin {
          animation: spinAppear 0.6s ease-out forwards;
        }
